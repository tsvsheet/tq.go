...
// the whole CLI — title header, NAME, the root query synopsis, DESCRIPTION,
// GLOBAL OPTIONS, and one .SS subsection per subcommand — dispatched through
// the command wiring.
func TestCLI_Man(t *testing.T) {
	out, err := runCLI(t, cmdMan)
	require.NoError(t, err)
	assert.Contains(t, out, `.TH TQ 1 "" "tq`)
	assert.Contains(t, out, ".SH NAME\ntq \\- Query TSV with a pipeline of relational verbs")
	assert.Contains(t, out, ".B tq\n[\\fIglobal options\\fR] <query> [file]\n.br\n")
	assert.Contains(t, out, ".SH DESCRIPTION")
	assert.Contains(t, out, ".SH GLOBAL OPTIONS")
	assert.Contains(t, out, ".SH COMMANDS")
	for _, sub := range []string{cmdComplete, cmdMan} {
...
// TestManPage_Bare proves a bare command — no args form, no flags, no
// subcommands — renders only the header, synopsis, and description skeleton.
func TestManPage_Bare(t *testing.T) {
	t.Parallel()

	page := manPage(&cli.Command{Name: "app"})
	assert.NotContains(t, page, ".br")
	assert.NotContains(t, page, ".SH GLOBAL OPTIONS")
	assert.NotContains(t, page, ".SH COMMANDS")
}
