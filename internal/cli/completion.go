...
// positional argument to `tq completion <shell>`.
type shellName string

// The shells for which tq emits a completion script, in help/message order.
// Each maps to a subcommand of urfave/cli's built-in (renamed) shell
...
to stdout. Source the output to enable completion of tq's commands and flags.

Examples:
  source <(tq completion bash)                            # .bashrc
  source <(tq completion zsh)                             # .zshrc
  tq completion fish > ~/.config/fish/completions/tq.fish`,
